...
class OrderFeedPage(BasePage):
    ...
    def feed_page_open_order(self):
        self.click_element_and_waiting_element_download(BasePageLocators.feed_button, OrderFeedPageLocators.top_order)
        top_order_in_feed = self.text_element(OrderFeedPageLocators.top_order_number_in_feed)
        self.click_element_and_waiting_element_download(OrderFeedPageLocators.top_order, OrderFeedPageLocators.modal_opened_section)
    ...
    def feed_order_in_history_exists_in_feed(self):
        order_number = self.place_order_get_number(OrderFeedPageLocators.ingredient_bun_link, OrderFeedPageLocators.ingredient_filling_link)
        self.click_element_and_waiting_element_download(BasePageLocators.feed_button, OrderFeedPageLocators.top_order)
        is_order_in_feed = self.is_displayed_order_in_feed(order_number)
        self.click_element_and_waiting_element_download(BasePageLocators.account_button, OrderFeedPageLocators.order_history_button)
        self.click_element_and_waiting_element_download(OrderFeedPageLocators.order_history_button, OrderFeedPageLocators.order_history_list)
        is_order_in_history = self.is_displayed_order_in_history(order_number)
        return is_order_in_feed, is_order_in_history

    @allure.step("При создании заказа счетчики Выполнено за всё время и Выполнено за сегодня - увеличиваются")
    def feed_page_counters_growth(self, counter):
        ...
        self.open_page_and_waiting_element_download(Urls.BASE_URL, OrderFeedPageLocators.ingredient_bun_link)
        order_number = self.place_order_get_number(OrderFeedPageLocators.ingredient_bun_link, OrderFeedPageLocators.ingredient_filling_link)
        self.click_element_and_waiting_element_download(BasePageLocators.feed_button, counter)
    ...
    @allure.step('После оформления заказа, номер появляется в разделе В работе')
    def feed_page_order_number_in_status_box(self):
        order_number = self.place_order_get_number(OrderFeedPageLocators.ingredient_bun_link, OrderFeedPageLocators.ingredient_filling_link)
        self.click_element_and_waiting_element_download(BasePageLocators.feed_button, OrderFeedPageLocators.status_box)
        order_number_in_in_process_box = OrderFeedPage.order_number_in_in_process_box_element(order_number)
        self.wait_order_in_box(order_number_in_in_process_box)
        return order_number